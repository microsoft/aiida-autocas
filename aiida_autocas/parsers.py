"""Parser for the AutoCAS calculation job."""
import re
...
class AutoCASParser(Parser):
    ...
    def parse(self, **kwargs):
        ...
        files_expected = [AutoCASCalculation.OUTPUT_FILE]
        ...
        with self.retrieved.open(AutoCASCalculation.OUTPUT_FILE, "r") as file:
            output = file.read()

        # Use regex to parse output
        # Parse Active space
        grp = re.search(
            r"Final\s*active\s*space\s*CAS\s*\(\s*e,\s*o\s*\)\s*:\s*\(\s*(\d+),\s*(\d+)\s*\)",
            output,
        )
        if grp:
